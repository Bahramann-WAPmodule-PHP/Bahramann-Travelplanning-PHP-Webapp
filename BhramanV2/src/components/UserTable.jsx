...
import {faAngleLeft, faUserPlus, faUsers} from '@fortawesome/free-solid-svg-icons';
            ...
            {showForm && (
              <div className="absolute right-0 mt-2 z-50 bg-white rounded-xl shadow-2xl p-6 w-80 border border-gray-200">
                <div className="flex justify-between items-center mb-2">
                  <h3 className="text-lg font-bold text-mainRed">{formMode === 'add' ? 'Add New User' : 'Edit User'}</h3>
                ...
                <form onSubmit={handleFormSubmit} className="space-y-3">
                  <div className="flex gap-2">
                    <div className="w-1/2">
                      ...
                      <input type="text" name="first_name" value={form.first_name} onChange={handleFormChange} className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-200" />
                    </div>
                    <div className="w-1/2">
                      <label className="block text-sm font-medium mb-1">Last Name</label>
                      <input type="text" name="last_name" value={form.last_name} onChange={handleFormChange} className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-200" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-medium mb-1">Email</label>
                    <input type="email" name="email" value={form.email} onChange={handleFormChange} className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-200" />
                  </div>
                  {formMode === 'add' && (
                    <div>
                      <label className="block text-sm font-medium mb-1">Password</label>
                      <input type="password" name="password" value={form.password} onChange={handleFormChange} className="w-full border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-red-200" />
                    </div>
                  )}
                  <div className="flex items-center gap-2">
                  ...
                  <button type="submit" className="w-full bg-mainRed hover:bg-red-600 text-white py-2 rounded-lg font-semibold shadow-md transition">{formMode === 'add' ? 'Add User' : 'Save Changes'}</button>
                </form>
              </div>
            )}
          </div>
                  ...
                  <React.Fragment key={user.id}>
                    <tr className={idx % 2 === 0 ? 'bg-white' : 'bg-red-50 hover:bg-red-100 transition'}>
                      ...
                      <td className="py-2 px-4 border-b border-gray-100">
                        <span className={user.is_admin ? 'bg-red-200 text-mainRed px-2 py-1 rounded-full text-xs font-semibold' : 'bg-gray-200 text-gray-700 px-2 py-1 rounded-full text-xs font-semibold'}>
